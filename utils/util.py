...
def save_hdrimg(image_path, image):
    cv2.imwrite(image_path + '.hdr', image)
    hdr_image = image
    hdr_tonemapped = cv2.createTonemap(gamma=2.6).process(hdr_image)
    hdr_tonemapped_scaled = np.clip(hdr_tonemapped * 255, 0, 255).astype('uint8')
    cv2.imwrite(image_path, hdr_tonemapped_scaled )
    return None
...
def rgb_to_gray(image):
    # 定义 REC709 转换矩阵
    rec709_coefficients = torch.tensor([0.0722, 0.7152, 0.2126])

    return torch.einsum('chw,c->hw', image, rec709_coefficients).unsqueeze(0)
